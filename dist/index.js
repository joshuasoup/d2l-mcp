...
function createServer() {
    ...
    const wrapToolHandler = (toolName, handler) => {
        return async (args) => {
            ...
            try {
                ...
                console.error(`[TOOL] Completed tool execution: ${toolName} (${elapsedTime}ms)`);
                return { content: [{ type: "text", text: result }] };
            }
            catch (error) {
                const elapsedTime = Date.now() - startTime;
                console.error(`[TOOL] Tool execution failed: ${toolName} (${elapsedTime}ms)`, error);
                throw error;
            }
        };
    };
    // Register assignment tools
    server.tool("get_assignments", assignmentTools.get_assignments.description, { orgUnitId: assignmentTools.get_assignments.schema.orgUnitId }, wrapToolHandler("get_assignments", async (args) => {
        return await assignmentTools.get_assignments.handler(args);
    }));
    server.tool("get_assignment", assignmentTools.get_assignment.description, {
        ...
        return await assignmentTools.get_assignment.handler(args);
    }));
    server.tool("get_assignment_submissions", assignmentTools.get_assignment_submissions.description, {
        ...
        return await assignmentTools.get_assignment_submissions.handler(args);
    }));
    // Register content tools
    server.tool("get_course_content", contentTools.get_course_content.description, { orgUnitId: contentTools.get_course_content.schema.orgUnitId }, wrapToolHandler("get_course_content", async (args) => {
        return await contentTools.get_course_content.handler(args);
    }));
    server.tool("get_course_topic", contentTools.get_course_topic.description, {
        ...
        return await contentTools.get_course_topic.handler(args);
    }));
    server.tool("get_course_modules", contentTools.get_course_modules.description, { orgUnitId: contentTools.get_course_modules.schema.orgUnitId }, wrapToolHandler("get_course_modules", async (args) => {
        return await contentTools.get_course_modules.handler(args);
    }));
    server.tool("get_course_module", contentTools.get_course_module.description, {
        ...
        return await contentTools.get_course_module.handler(args);
    }));
    // Register grade tools
    server.tool("get_my_grades", gradeTools.get_my_grades.description, { orgUnitId: gradeTools.get_my_grades.schema.orgUnitId }, wrapToolHandler("get_my_grades", async (args) => {
        return await gradeTools.get_my_grades.handler(args);
    }));
    ...
    server.tool("get_upcoming_due_dates", calendarTools.get_upcoming_due_dates.description, {
        ...
        return await calendarTools.get_upcoming_due_dates.handler(args);
    }));
    // Register news tools
    server.tool("get_announcements", newsTools.get_announcements.description, { orgUnitId: newsTools.get_announcements.schema.orgUnitId }, wrapToolHandler("get_announcements", async (args) => {
        return await newsTools.get_announcements.handler(args);
    }));
    // Register enrollment tools
    server.tool("get_my_courses", enrollmentTools.get_my_courses.description, {}, wrapToolHandler("get_my_courses", async () => {
    ...
    server.tool("download_file", "Download a file from D2L Brightspace. Provide a D2L content URL (e.g., https://learn.ul.ie/content/enforced/68929-CS4444.../file.docx or /content/enforced/...). The file will be saved to your Downloads folder by default, or to a custom path if specified. Returns the local file path, filename, size, and content type. Use this to download lecture slides, assignment files, course materials, or any file linked in course content.", {
        url: z
            .string()
            .describe("The D2L URL or path to the file to download (e.g., https://learn.ul.ie/content/enforced/68929-CS4444_SEM1_2025_6/file.docx)"),
        savePath: z
            .string()
            .optional()
...
async function main() {
    ...
    if (transportType === "stdio") {
        ...
        const mcpPostHandler = async (req, res) => {
            ...
            console.error(`[MCP] Session ID: ${sessionId || "none (initialization)"}`);
            console.error(`[MCP] Method: ${requestMethod}`);
            console.error(`[MCP] Request ID: ${requestId}`);
            if (requestMethod === "tools/call") {
                console.error(`[MCP] Tool: ${requestParams.name || "unknown"}, Args:`, JSON.stringify(requestParams.arguments || {}));
            }
            try {
                let transport;
                if (sessionId && transports[sessionId]) {
                    ...
                    transport = new StreamableHTTPServerTransport({
                        ...
                        onsessioninitialized: (sessionId) => {
                            ...
                            console.error(`[MCP] Session initialized with ID: ${sessionId} (${initTime}ms)`);
                    ...
                    transport.onclose = () => {
                        ...
                        if (sid && transports[sid]) {
                            console.error(`[MCP] Transport closed for session ${sid}, removing from transports map`);
                    ...
                    console.error(`[MCP] Server connected to transport (${connectTime}ms)`);
                    const handleStartTime = Date.now();
                    await transport.handleRequest(req, res, req.body);
                    const handleTime = Date.now() - handleStartTime;
                    const totalTime = Date.now() - requestStartTime;
                    console.error(`[MCP] Request handled (${handleTime}ms, total: ${totalTime}ms)`);
                    return; // Already handled
                }
                else {
                    // Invalid request - no session ID or not initialization request
                    const totalTime = Date.now() - requestStartTime;
                    console.error(`[MCP] Invalid request - no session ID or not initialization (${totalTime}ms)`);
                    res.status(400).json({
                        jsonrpc: "2.0",
                        error: {
                            code: -32000,
                ...
                const totalTime = Date.now() - requestStartTime;
                console.error(`[MCP] Error handling MCP request (${totalTime}ms):`, error);
                if (!res.headersSent) {
                    res.status(500).json({
                        jsonrpc: "2.0",
                        error: {
        ...
        const mcpGetHandler = async (req, res) => {
            ...
            if (lastEventId) {
                ...
                console.error(`[MCP] Establishing new SSE stream for session ${sessionId}`);
            }
            ...
            const handleTime = Date.now() - handleStartTime;
            const totalTime = Date.now() - requestStartTime;
            console.error(`[MCP] GET request handled (${handleTime}ms, total: ${totalTime}ms)`);
        };
        ...
        const mcpDeleteHandler = async (req, res) => {
            ...
            console.error(`[MCP] Received session termination request for session ${sessionId}`);
            try {
                const transport = transports[sessionId];
                const handleStartTime = Date.now();
                await transport.handleRequest(req, res);
                const handleTime = Date.now() - handleStartTime;
                const totalTime = Date.now() - requestStartTime;
                console.error(`[MCP] DELETE request handled (${handleTime}ms, total: ${totalTime}ms)`);
            }
            catch (error) {
                const totalTime = Date.now() - requestStartTime;
                console.error(`[MCP] Error handling session termination (${totalTime}ms):`, error);
                if (!res.headersSent) {
        ...
        console.error(`Invalid MCP_TRANSPORT value: ${transportType}. Must be 'stdio', 'http', or 'https'`);
        process.exit(1);
    }
}
main().catch((error) => {
